...
export default function ProfilePage() {
  ...
  if (loading || partnerLoading) {
    return (
      <div className="min-h-[50vh] flex items-center justify-center font-black uppercase text-slate-400">
        Ачааллаж байна...
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto space-y-16">
        <div className="flex flex-col md:flex-row items-center gap-10">
          <div className="w-32 h-32 bg-white rounded-[2.5rem] shadow-2xl flex items-center justify-center border-4 border-white overflow-hidden">
            {user.photoURL ? (
              <Image
                src={user.photoURL}
                alt={user.displayName || "Хэрэглэгч"}
                width={128}
                height={128}
                sizes="128px"
        ...
        {approvedPartner && (
          <div className="bg-white rounded-[2.5rem] shadow-xl border border-white p-10">
            <p className="text-[10px] font-black uppercase tracking-widest text-emerald-700 mb-3">Партнер самбар</p>
            <div className="flex flex-col md:flex-row md:items-center gap-6">
              <div className="w-24 h-24 rounded-2xl bg-slate-50 border border-slate-100 overflow-hidden flex items-center justify-center">
                {approvedPartner.logo ? (
                  <Image
                    ...
                    alt={approvedPartner.name || "Партнер"}
                    width={96}
                    height={96}
                    sizes="96px"
                    className="w-full h-full object-cover"
              ...
              <div className="flex-1">
                <h2 className="text-3xl font-black text-slate-900 italic tracking-tight">
                  {approvedPartner.name || "Партнер"}
                </h2>
                <p className="text-slate-500 mt-2">
                  {approvedPartner.description || "Танилцуулга нэмэгдээгүй байна."}
                </p>
              </div>
            </div>
        ...
        <Card className="rounded-[3rem] border-none shadow-xl p-10 bg-white">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Хадгалсан тэтгэлэг</p>
            </div>
          </div>
        </Card>
                ...
                <Card key={item.id} className="rounded-[2.5rem] border-none shadow-sm bg-white hover:shadow-md transition-all group">
                  <CardContent className="p-6 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-6 min-w-0">
                      <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center text-xl shrink-0">
                        {flagForCountry(item.country)}
                      </div>
                      <div className="min-w-0">
                        <h4 className="font-bold text-slate-900 truncate">{item.title}</h4>
                        <p className="text-slate-400 text-sm">{item.category || item.country}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <Link href={`/scholarships/${item.id}`}>
                        <Button className="bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded-xl" aria-label="Дэлгэрэнгүй">
                          <ArrowRight size={20} />
                        </Button>
                      </Link>
                    </div>
